...
object WikipediaRanking {
  ...
  def containsLang(article: WikipediaArticle, lang: String): Boolean = {
...
//    rdd.aggregate(0)((agg, article) => if (containsLang(article, lang)) agg + 1 else agg, _+_)
    rdd.filter(containsLang(_, lang)).count().toInt
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd
      .flatMap(article => langs.filter(containsLang(article, _)).map(lang => (lang, article)))
      .groupByKey()
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd
      .flatMap(article => article.text.split(" ").distinct.filter(langs.contains(_)).map(lang => (lang, 1)))
      .reduceByKey(_+_)
      .collect
      .sortBy(-_._2)
      .toList
